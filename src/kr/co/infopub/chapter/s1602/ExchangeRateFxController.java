...
public class ExchangeRateFxController {
    ...
    void onConvertAction(ActionEvent event) {
		...
		String me="KRW";       //한화
		String kind=you+me;     //USDKRW 미국의 달러를 한화로 
		Finance finance=USDConvert.convert(kind); //비율을 얻어서
		tfusd.setText(finance.rate);   
		//일본 엔
		you="JPY";
		kind=you+me;     //JPYKRW 인본의 엔을 한화로 
		finance=USDConvert.convert(kind); //비율을 얻어서
		tfjpy.setText(finance.rate); 
		//중국 위엔
		you="CNY";
		kind=you+me;     //CNYKRW 중국의 위엔을 한화로 
		finance=USDConvert.convert(kind); //비율을 얻어서
		tfcny.setText(finance.rate); 
    }
    //라디오-다른 국가의 화폐로 환전
    @FXML
